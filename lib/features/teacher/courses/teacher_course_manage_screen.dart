...
class _TeacherCourseManageScreenState extends State<TeacherCourseManageScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _assignments = _list(results[2])
          .where((a) =>
              (a['course_id'] ?? '').toString() == widget.courseId)
          .toList();
      _exams = _list(results[3])
          .where((e) => (e['course_id'] ?? '').toString() == widget.courseId)
          .toList();
    } catch (_) {
      Get.snackbar('خطأ', 'تعذّر تحميل بيانات الدورة',
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(_isEnded ? 'منتهية' : 'نشطة',
    ...
    return [
      ...
      TeacherDashboardCard(
        ...
        child: Column(
          children: [
            Row(children: [
              _statTile(context, 'الطلاب', '${_students.length}',
                  Icons.group_outlined, mq.success),
              const SizedBox(width: MqSpacing.sm),
              _statTile(context, 'الحصص', '${_sessions.length}',
                  Icons.event_note_outlined, mq.accent),
            ]),
            const SizedBox(height: MqSpacing.sm),
            Row(children: [
              _statTile(context, 'الواجبات', '${_assignments.length}',
                  Icons.assignment_outlined, mq.orange),
              const SizedBox(width: MqSpacing.sm),
              _statTile(context, 'الاختبارات', '${_exams.length}',
                  Icons.quiz_outlined, mq.error),
            ]),
          ],
        ),
      ),
  ...
  List<Widget> _studentsTab(BuildContext context) {
    if (_students.isEmpty) {
      return [
        _emptyCard(context, 'لا يوجد طلاب في هذه الدورة بعد',
  ...
  void _openStudentDetail(Map<String, dynamic> s) {
    ...
    Get.to(() => TeacherStudentDetailScreen(
          studentId: id,
  ...
  List<Widget> _attendanceTab(BuildContext context) {
    if (_sessions.isEmpty) {
      return [
        _emptyCard(context, 'لا توجد حصص لهذه الدورة بعد',
  ...
  List<Widget> _assignmentsTab(BuildContext context) {
    return [
      ...
      if (_assignments.isEmpty)
        _emptyCard(context, 'لا توجد واجبات لهذه الدورة بعد',
        ...
        for (final a in _assignments)
          Padding(
            ...
            child: _AssignmentRow(
                assignment: a, onTap: () => _openAssignmentDetail(a)),
          ),
    ];
  }
  ...
  Future<void> _openAssignmentDetail(Map<String, dynamic> a) async {
    final changed = await Get.to<bool>(() => TeacherAssignmentDetailScreen(
  ...
  List<Widget> _examsTab(BuildContext context) {
    return [
      ...
      const SizedBox(height: MqSpacing.md),
      if (_exams.isEmpty)
        _emptyCard(context, 'لا توجد اختبارات لهذه الدورة بعد',
            Icons.quiz_outlined)
      else
  ...
  Future<void> _openExamDetail(Map<String, dynamic> e) async {
    final changed = await Get.to<bool>(() => TeacherExamDetailScreen(
  ...
  Future<void> _loadEvalStudents() async {
    ...
    try {
      final res = await _api.fetchEvaluationStudents(
          widget.courseId, _dateStr(_evalDate));
  ...
  List<Widget> _evaluationsTab(BuildContext context) {
    ...
    return [
      Row(
        children: [
          Expanded(
            child: Text('تقييم الطلاب', style: context.text.titleSmall),
          ),
          InkWell(
            onTap: _pickEvalDate,
            borderRadius: MqRadius.brPill,
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.calendar_today_outlined, size: 14, color: mq.accent),
                const SizedBox(width: 4),
                Text(isToday ? 'اليوم' : _dateStr(_evalDate),
        ...
        for (final s in _evalStudents)
          Padding(
            ...
            child: _EvalStudentRow(
                student: s, onTap: () => _evaluateStudent(s)),
          ),
    ];
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                width: 46,
                decoration:
                    BoxDecoration(color: soft, borderRadius: MqRadius.brMd),
...
class _StudentRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final phone =
        (student['student_phone'] ?? student['phone'] ?? '').toString();
...
class _SessionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final mq = context.mq;
    final title =
        (session['title'] ?? session['course_name'] ?? 'حصة').toString();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(
                  [day, if (start.isNotEmpty) '$start - $end']
