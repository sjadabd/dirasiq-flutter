...
class _TeacherDetailsScreenState extends State<TeacherDetailsScreen> {
  ...
  String get _name =>
      (_teacher?['name'] ?? _teacher?['full_name'] ?? '').toString().trim();
  String? get _photo {
    final p =
        (_teacher?['profileImagePath'] ??
                _teacher?['profile_image_path'] ??
                _teacher?['image'] ??
                _teacher?['avatar'])
            ?.toString();
  ...
  String? get _specialization {
    ...
    final s = (_teacher?['specialization'] ?? _teacher?['bio'])
        ?.toString()
        .trim();
  ...
  double? get _rating {
    final r =
        _teacher?['rating'] ??
        _teacher?['avgRating'] ??
        _teacher?['average_rating'];
    return r is num ? r.toDouble() : double.tryParse('${r ?? ''}');
  }

  bool get _verified =>
      _teacher?['isVerified'] == true ||
      _teacher?['verified'] == true ||
      _teacher?['is_verified'] == true;

  String? get _bio {
    final b =
        (_teacher?['bio'] ?? _teacher?['about'] ?? _teacher?['description'])
            ?.toString()
            .trim();
  ...
  bool get _hasLocation =>
      _toDouble(_teacher?['latitude']) != null &&
      ...
      child: Directionality(
        ...
        child: Builder(
          builder: (context) => Scaffold(
            ...
            appBar: AppBar(
              title: Text(_name.isEmpty ? 'تفاصيل المعلّم' : _name),
            ),
          ...
          if (_activeCourses.isEmpty)
            ...
            for (final c in _activeCourses)
              Padding(
                ...
                child: _activeCourseCard(context, c),
              )
  ...
  Widget _activeCourseCard(BuildContext context, Map<String, dynamic> c) {
    ...
    final progressRaw =
        c['progressPercent'] ?? c['attendancePercent'] ?? c['progress'];
    final progress = progressRaw is num
        ? progressRaw.toDouble()
        : double.tryParse('${progressRaw ?? ''}');
    final nextRaw =
        c['nextLecture'] ?? c['nextOccurrence'] ?? c['next_lecture'];
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          if (stage != null && stage.isNotEmpty)
                            MqBadge(label: stage, tone: MqBadgeTone.neutral),
                        ],
                      ),
                    ],
                  ],
          ...
          if (next != null) ...[
            ...
            Row(
              children: [
                ...
                MqSpacing.gapXxs,
                Text(
                  'المحاضرة القادمة: ${formatDateTime12(next).replaceFirst(' ', ' • ')}',
  ...
  Widget _assignmentCard(BuildContext context, Map<String, dynamic> a) {
    ...
    final dueStr = due == null
        ? null
        : (DateTime.tryParse(due.toString()) != null
              ? DateFormat(
                  'dd/MM/yyyy',
                ).format(DateTime.parse(due.toString()).toLocal())
              : due.toString());
    final st = _subStatus(
      (a['submissionStatus'] ?? a['status'] ?? 'pending').toString(),
    );
    return _infoRow(
      context,
      Icons.edit_note_outlined,
      mq.accent,
      title,
      dueStr == null ? null : 'موعد التسليم: $dueStr',
      st,
    );
  }
  ...
  Widget _examCard(BuildContext context, Map<String, dynamic> e) {
    ...
    final title = type == 'monthly'
        ? 'امتحان شهري'
        : (type == 'daily' ? 'امتحان يومي' : 'امتحان');
    ...
    return _infoRow(
      ...
      mq.orange,
      title,
      dateStr == null ? null : 'التاريخ: $dateStr',
      null,
    );
  }
  ...
  Widget _alertCard(BuildContext context, Map<String, dynamic> al) {
    final mq = context.mq;
    final title =
        (al['title'] ?? al['message'] ?? al['body'] ?? al['text'] ?? 'إعلان')
            .toString();
              ...
              child: SizedBox(
                ...
                child: _photo == null
                    ? Container(
                        ...
                        child: Text(
                          _name.isNotEmpty ? _name.characters.first : '؟',
  ...
  Widget _courseCard(BuildContext context, Map<String, dynamic> c) {
    final mq = context.mq;
    final name = (c['course_name'] ?? c['courseName'] ?? c['name'] ?? '')
        .toString();
    ...
    final img = (imgRaw == null || imgRaw.isEmpty)
        ...
        : (imgRaw.startsWith('http')
              ? imgRaw
              : '${AppConfig.serverBaseUrl}$imgRaw');
    final subject = c['subject'] is Map
        ? (c['subject']['name'] ?? '').toString()
        : (c['subject_name'] ?? '').toString();
    final grade = (c['gradeName'] ?? c['grade_name'] ?? c['grade'])?.toString();
    final price = c['price'];
    final priceStr = price == null
        ? null
        : '${NumberFormat('#,###').format(_toDouble(price) ?? 0)} د.ع';
    final type = _courseType(c);
    final id = (c['id'] ?? '').toString();
          ...
          ClipRRect(
            ...
            child: SizedBox(
              ...
              child: img == null
                  ? Container(
                      color: mq.fill2,
                      child: Icon(Icons.menu_book_rounded, color: mq.ink3),
                    )
                  : Image.network(
                      img,
                      fit: BoxFit.cover,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (grade != null && grade.isNotEmpty)
                      MqBadge(label: grade, tone: MqBadgeTone.neutral),
                    if (priceStr != null)
                      MqBadge(label: priceStr, tone: MqBadgeTone.orange),
                  ],
                ),
              ],
            ),
  ...
  String? _courseType(Map<String, dynamic> c) {
    final raw =
        (c['courseType'] ?? c['course_type'] ?? c['type'] ?? c['delivery'])
            ?.toString()
            .toLowerCase();
